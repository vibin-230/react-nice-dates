...
function getGame(res,convo_id,refund_status,next,req){
  Conversation.findById({_id:convo_id}).lean().populate('members','_id name device_token profile_picture handle name_status visibility sports_interest').then((convo)=>{
    ...
    Game.findOne({conversation:convo_id}).lean().populate("mvp.target_id","name handle profile_picture _id").populate("conversation").populate('host','_id name profile_picture phone handle name_status visibility sports_interest').populate('users','_id name profile_picture phone handle name_status visibility sports_interest').populate('invites','_id name profile_picture phone handle visibility sports_interest').then(game=>{
    ...
    Message.updateMany({conversation:req.body.conversation_id,read_status:false},{ '$set': { "read_status" : true } },{multi:true}).then((user)=>{
      const a =  convo && convo.last_active && convo.last_active.length > 0 ? convo.last_active.map((con)=>{
        if(con.user_id.toString() === req.params.id.toString()){
          ...
          Conversation.findById({_id:req.body.conversation_id}).then(conversation=>{
...
router.post('/friend_get_following/:id', [
  ...
  User.findOne({_id:req.params.id},{activity_log:0}).populate("followers","name _id handle name_status profile_picture visibility").populate("following","name _id handle name_status profile_picture visibility").lean().then(user1 => {
    ...
    Conversation.findByIdAndUpdate({_id: req.body.game.conversation},{ $addToSet: { invites: { $each: req.body.ids } } }).then(conversation=> {
      User.find({_id: { $in :req.body.ids } },{activity_log:0}).lean().then(user=> {
        ...
        res.send({status:"success", message:"invitation sent"})
          NotifyArray(device_token_list,'You have a received a new game request from '+req.name,`${game.name}`)
}).catch(next);
}).catch(next);
}).catch(next);
});
  ...
  console.log('filter',filter,query)
   User.find({_id:convo.created_by}).select('name -_id').then(user=>{
    ...
    Message.find({$and:[{ "game": { $exists: true, $ne: null }},{conversation:req.body.conversation_id},query]}).distinct("game").then((games)=>{
      Game.find({_id:{ $in: games}}).populate('conversation').populate({path:'venue', model:'venue',select:'venue'}).lean().then(gameinfo=>{
...
router.post('/book_slot', verifyToken, (req, res, next) => {
  function BookSlot(body,id){
    return new Promise(function(resolve, reject){
      Booking.findByIdAndUpdate({_id:body._id},{booking_status:"booked",old_booking:true, transaction_id:body.transaction_id, booking_amount:body.booking_amount,coupon_amount:body.coupon_amount,coupons_used:body.coupons_used, multiple_id:id}).lean().then(booking=>{
        Booking.findById({_id:body._id}).lean().populate('venue_data').then(booking=>{
        resolve(booking)
      }).catch(next)
    }).catch(next)
...
async function handleSlotAvailabilityForGames1(booking1,client){
  let booking = booking1[0]
  const slot_time = { $in: booking1.map((b)=>b.slot_time) }
  const x =  await  Booking.find({  venue_id:booking.venue_id, booking_date:booking.booking_date,slot_time:slot_time,booking_status:{$in:["blocked","booked","completed"]}}).lean().then(booking_history=>{
    let promisesToRun = [];
               ...
               return Game.updateMany({"bookings.booking_id":{$nin:[booking.booking_id]},"booking_date":booking.booking_date,"bookings.booking_status":{$in:['blocked','hosted','cancelled']},"bookings.venue_id":booking.venue_id,"bookings.slot_time":slot_time,status:true },{$set:{status:false,status_description:'Sorry! Slot has been booked by some other user'}}).lean().then(game1=>{
                ...
                return Message.insertMany(messages).then(message1=>{
                  ...
                  const cids = m.map((entry)=>{
                    ...
                    const device_token_list=user.map((e)=>e.device_token)
                                                  NotifyArray(device_token_list,'Apologies! This game has been cancelled as the slot has been booked by another user. Please choose another slot to host your game.',`${game1.name}`)
                                                    return 'pass'
                 }).catch((e)=>console.log(e));
              }).catch(error => console.log(error))
            }).catch(error => console.log(error))
...
router.post('/book_slot_and_host', verifyToken, (req, res, next) => {
  function BookSlot(body,id){
    return new Promise(function(resolve, reject){
      Booking.findByIdAndUpdate({_id:body._id},{booking_status:"booked",old_booking:false, transaction_id:body.transaction_id, booking_amount:body.booking_amount,coupon_amount:body.coupon_amount,coupons_used:body.coupons_used, multiple_id:id,game:true,coins:body.coins}).lean().then(booking=>{
        Booking.findById({_id:body._id}).lean().populate('venue_data').then(booking=>{
        resolve(booking)
      }).catch(next)
    }).catch(next)
  ...
  Promise.all(promisesToRun).then(values => {
   ...
   if(req.body[0].transaction_id && req.body[0].transaction_id !== 'free_slot'){
    ...
    Admin.find({venue:{$in:[values[0].venue_id]},notify:true},{activity_log:0}).then(admins=>{
      ...
      // let to_mail = `${values[0].email}, bookings@turftown.in,bookings@turftown.in`
      // console.log(mailBody)
      // ejs.renderFile('views/mail.ejs',mailBody).then(html=>{
      //   mail("bookings@turftown.in", to_mail,"Venue Booked","test",html,response=>{
      //     if(response){
...
router.post('/book_slot_for_admin1/:id', verifyToken, AccessControl('booking', 'create'), (req, res, next) => {
...
router.post('/checkMobileForLogin', (req, res, next) => {
    ...
    User.find({$and:[{"phone":{ "$regex": req.body.mobile, "$options": "i" }},{'email':{$exists:true,$ne:null }} ]},{__v:0,token:0,otp:0,activity_log:0}).then(user=>{
    if(user && user.length > 0){
...
router.post('/test_textlocal', verifyToken, (req, res, next) => {
  ...
  User.find({phone:req.body.phone},{activity_log:0}).lean().then((u)=>{
    Conversation.find({ $or: [ { members: { $in: [u[0]._id] } },{ exit_list: { $elemMatch: {user_id:[u[0]._id]} } }] }).lean().populate("host","name profile_picture handle name_status").populate('to',' name _id profile_picture last_active online_status status handle name_status visibility').populate('members','name _id profile_picture last_active online_status status handle name_status visibility').populate('exit_list.user_id','name _id profile_picture last_active online_status status handle name_status visibility').populate('last_message').limit(10).then(existingConversation=>{
      let chatroom = existingConversation[0]
    ...
    console.log(u[0].handle,u[0].device_token,existingConversation.length);
    
    notify(u[0],'total',chatroom)
 ...
 // console.log(body,id)
  return new Promise((resolve,reject)=>{
    Venue.findById({_id:id},{bank:0,access:0}).lean().then(venue=>{
      ...
      // Booking.find({$and:[{venue:body.venue, venue_id:id, booking_date:{$gte:body.booking_date,$lt:moment(body.booking_date).add(1,"days")}}],booking_status:{$in:["booked","blocked","completed"]}}).then(booking_history=>{
        let slots_available = SlotsAvailable(venue,booking_history)
        console.log('sa',slots_available.slots_available);
        console.log('body slot time',body.slot_time);
          
        if(slots_available.slots_available[body.slot_time][body.venue_type] > 0){
          resolve(slots_available.slots_available[body.slot_time][body.venue_type])
        }else{
...
function updateGameStatusAndGetMessages(nc,status){
...
function updateConversationsAndGames(game,messages,booking){
  const members = _.flatten(game.map((g)=>g.conversation.members))
                 return   User.find({_id: { $in :members } },{activity_log:0}).lean().then(user=> {
                   return Message.insertMany(messages).then(message1=>{
                   const message_ids = message1.map((m)=>m._id)
                 ...
                 return  Booking.updateMany({booking_id:booking.booking_id,user_id:booking.user_id},{$set:{game:false}},{multi:true}).then(booking=>{
                   //const device_token_list=user.map((e)=>e.device_token)
      ...
      console.log(slots_available.slots_available);
        ...
        return game2
          ...
          return Game.updateMany({_id:{$in:ids}},{$set:{status:true,status_description:'',booking_status:'hosted'}}).lean().then(game1=>{
            ...
            updateConversationsAndGames(s,messages,booking)
            return 'status updated'
    }).catch((e)=>console.log(e));
  }else{
    return 'games part update'
  }
  }else{
    return 'no games'
  }
...
router.post('/cancel_booking/:id', verifyToken, (req, res, next) => {
  Booking.findOne({booking_id:req.params.id,user_id:req.userId}).then(booking=>{
    User.findById({_id:req.userId}).then(user=>{
              ...
              Booking.updateMany({booking_id:req.params.id,user_id:req.userId},{$set:{booking_status:"cancelled", refunded: true,refund_status:true,game:false,description:'from_game_with_refund'}},{multi:true}).then(booking=>{
                Booking.find({booking_id:req.params.id,user_id:req.userId}).lean().populate('venue_data').then(booking=>{
                  ...
                  ejs.renderFile('views/event_manager/venue_cancel.ejs',obj).then(html=>{
                    let to_emails = `${user.email}, kumar@turftown.in,ashok@turftown.in`
                    console.log('to_emails',to_emails);
                    mail("kumar@turftown.in", to_emails,booking_id+" has been cancelled","Slot Cancellation",html,response=>{
                      if(response){
                  ...
                  //   mail("bookings@turftown.in", manager_mail,booking_id+" has been cancelled","Slot Cancellation",html,response=>{
                  //     if(response){
                  //       res.send({status:"success"})
                  //     }else{
                  //       res.send({status:"failed"})
...
router.post('/get_followers_and_following', verifyToken, (req, res, next) => {
  User.findOne({_id:req.userId},{activity_log:0}).lean().populate('following','visibility name phone profile_picture handle name_status').populate('followers','visibility name phone profile_picture handle name_status').then(user=>{
...
router.post('/get_followers_and_following_for_user', verifyToken, (req, res, next) => {
  User.findOne({_id:req.body.id},{activity_log:0}).lean().populate('following','visibility name phone profile_picture handle name_status').populate('followers','visibility name phone profile_picture handle name_status').then(user=>{
    const following = user.following
  //   .map((a)=>{
  //     a['select'] = false
  //     return a
...
router.post('/cancelled_booking_history_by_time/:id', verifyToken, (req, res, next) => {
  Venue.findById({_id:req.params.id},{bank:0,access:0}).lean().then(venue=>{
    ...
    Booking.find({booking_status:{$in:["cancelled"]},venue_id:{$in:venue_id}, booking_date:{$gte:req.body.fromdate, $lte:req.body.todate}, start_time:{$gte:req.body.start_time},end_time:{$lte:req.body.end_time}}).then(bookings=>{
      ...
      Booking.find({booking_id:{$in:booking_ids},venue_id:req.params.id,repeat_booking:false,booking_status:{$in:["cancelled"]}}).lean().populate('cancelled_by','name').then(booking=>{
      Booking.find({booking_id:{$in:booking_ids},venue_id:req.params.id,repeat_booking:true,booking_status:{$in:["cancelled"]}}).lean().populate('cancelled_by','name').then(bookings=>{
        result = Object.values(combineSlots(booking))
...
router.post('/booking_history_by_time/:id', verifyToken, (req, res, next) => {
  Venue.findById({_id:req.params.id},{bank:0,access:0}).lean().then(venue=>{
    ...
    Booking.find({booking_status:{$in:["booked","completed"]},venue_id:{$in:venue_id}, booking_date:{$gte:req.body.fromdate, $lte:req.body.todate}, start_time:{$gte:req.body.start_time},end_time:{$lte:req.body.end_time}}).then(bookings=>{
      ...
      Booking.find({booking_status:{$in:["booked","completed"]},booking_id:{$in:booking_ids},repeat_booking:false,venue_id:req.params.id}).lean().populate('collected_by','name').then(bookings=>{
        Booking.find({booking_status:{$in:["booked","completed"]},booking_id:{$in:booking_ids},repeat_booking:true,venue_id:req.params.id}).lean().populate('collected_by','name').then(booking=>{
  ...
  EventBooking.findOne({event_id: req.body.event_id, created_by: req.userId,booking_status:'booked'}).then(event=>{
    ...
    if(event){
      ...
      EventBooking.find({event_id:req.body.event_id,booking_status:'booked'}).lean().populate('event_id').then(bookingOrders=>{
        if(bookingOrders && bookingOrders.length > 0 && bookingOrders[0].event_id && bookingOrders.length<bookingOrders[0].event_id.format.noofteams){
          if(bookingOrders && bookingOrders.length > 0 &&  bookingOrders[0].event_id && bookingOrders[0].event_id.status){
...
router.post('/cancel_event_booking/:id', verifyToken, (req, res, next) => {
  ...
  EventBooking.findOne({booking_id:req.params.id}).lean().populate('event_id').then(eventBooking=>{
    ...
    // let to_emails_manager = `${event_email}, bookings@turftown.in`

    // let to_emails = `${eventBooking.email}, bookings@turftown.in`


    // ejs.renderFile('views/event_manager/event_cancel.ejs',mailBody).then(html=>{
    //   mail("bookings@turftown.in", to_emails,"Event "+booking_id+" has been cancelled","test",html,response=>{
    //     if(response){
    //       console.log('success')
    //     }else{
    //       console.log('failed')
    ...
    //   mail("bookings@turftown.in", to_emails_manager,"Event "+booking_id+" has been cancelled for "+mailBody.event_name,"test",html,response=>{
    //     if(response){
    //       console.log('success')
    //     }else{
    //       console.log('failed')
          ...
          EventBooking.findOne({}, null, {sort: {$natural: -1}}).lean().populate('event_id').then(bookingOrder=>{
            ...
            EventBooking.create(booking_data).then(eventBooking=>{
             ...
             // SendMessage(phone,sender,EVENT_BOOKED_USER) /// user sms
              ...
              //   mail("bookings@turftown.in", to_emails,"New registeration for "+event_name,"test",html,response=>{
              //     if(response){
              //       console.log('success')
              //     }else{
              //       console.log('failed')
