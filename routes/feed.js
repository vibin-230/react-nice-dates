...
router.post('/get_town_games/', [
  ...
  router.post('/get_alerts/', [
    ...
    Alert.find({user: req.userId},{}).lean().populate({ path: 'game', populate: { path: 'conversation' , populate :{path:'last_message'} } }).populate({ path: 'post', populate: { path: 'game' , populate :{path:'conversation',populate :{path:'last_message'}} } }).populate('created_by','name _id handle profile_picture').then(alert=> {
      console.log(alert)    
