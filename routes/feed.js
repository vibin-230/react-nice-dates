      ...
      Post.findByIdAndUpdate({_id: req.params.id},filter ).then(game=> {
        Post.findOne({_id: req.params.id}).lean().populate({path:"event",populate:{path:"venue",select:"venue"}}).populate('shout_out','_id name profile_picture phone handle name_status').populate('created_by','_id name profile_picture phone handle name_status').populate({ path: 'game', populate: [{ path: 'conversation' , populate :{path:'last_message'} },{path:'host',select:'_id name profile_picture phone handle name_status'},{path:'users',select:'_id name profile_picture phone handle name_status'},{path:'invites',select:'_id name profile_picture phone handle name_status'},{path:'venue',select:'venue'}] }).then((s)=>{
          if(s.created_by._id.toString() !== req.userId.toString()){
            !req.body.status && sendAlert({created_at:new Date(),created_by:req.userId,user:s.created_by,post:s._id,type:'shoutout',status_description:`${user.handle} gave you a shoutout.`},'create',next)
            req.body.status && sendAlert({created_at:new Date(),created_by:req.userId,user:s.created_by._id,post:s._id,type:'shoutout',status_description:`${user.handle} remove your a shoutout.`},'delete',next)
...
router.post('/get_town_games/', [verifyToken,], (req, res, next) => {
    User.findOne({_id: req.userId,'handle':{$exists:true,$ne:null }}).lean().then(user=> {
      ...
      Post.find(filter1).lean().populate('shout_out','_id name profile_picture phone handle name_status visibility').populate({path:"event",populate:{path:"venue"}}).populate('created_by','_id name profile_picture phone handle name_status visibility').populate({ path: 'game', populate: [{ path: 'conversation' , populate :{path:'last_message'} },{path:'host',select:'_id name profile_picture phone handle name_status visibility'},{path:'users',select:'_id name profile_picture phone handle name_status visibility'},{path:'invites',select:'_id name profile_picture phone handle name_status visibility'},{path:'venue',select:'venue'}] }).then((posts)=>{
        Post.find({shout_out: { $in: [req.params.id] } ,status:true}).lean().populate('shout_out','_id name profile_picture phone handle name_status visibility').populate({path:"event",populate:{path:"venue"}}).populate('created_by','_id name profile_picture phone handle name_status visibility').populate({ path: 'game', populate: [{ path: 'conversation' , populate :{path:'last_message'} },{path:'host',select:'_id name profile_picture phone handle name_status visibility'},{path:'users',select:'_id name profile_picture phone handle name_status visibility'},{path:'invites',select:'_id name profile_picture phone handle name_status visibility'},{path:'venue',select:'venue'}] }).then((shouted_posts)=>{
...
router.post('/get_more_alerts/', [
  ...
  router.post('/tt_users/', [
    ...
    User.find({_id:{$nin:[req.userId]},handle:{"$exists" : true, "$ne" : ""}}).select("name profile_picture handle name_status _id visibility").then(data=>{
        res.status(201).send({status: "success", message: "user collected",data:data})
      }).catch(next)
  });
  ...
  router.post('/get_coin_history_admin/', [
    verifyToken,
        ...
        Coins.find({}).populate("from","name profile_picture handle name _id phone").populate("user","name profile_picture handle _id phone").then(exp=> {
          if (exp) {
            res.status(201).send({status: "success", message: "coin history collected",data:exp})
          } else {
              res.status(422).send({status: "failure", errors: {user:"force update failed"}});
