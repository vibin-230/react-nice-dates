...
module.exports =  function slotsAvailableWithNoOfCourts(venue,booking_history){
  if(venue.configuration.convertable){
    ...
    if(booking_history.length>0){
      let available_inventory = Object.values(booking_history).map(booking =>{
        ...
        inventory[base_type] = parseInt(inventory[base_type] - conf.ratio[booking.venue_type] *
          (booking.courts == null || booking.courts == undefined
            ? 1
            : booking.courts))
    ...
    if(booking_history.length>0){
      let available_inventory = Object.values(booking_history).map(booking =>{
        ...
        if(!slots_available[booking.slot_time]){
          slots_available[booking.slot_time] = inventory
          slots_available[booking.slot_time][booking.venue_type] = parseInt(inventory[booking.venue_type] - 1*((booking.courts == null || booking.courts == undefined) ? 1 : booking.courts ))
        }else if(slots_available[booking.slot_time]){
          slots_available[booking.slot_time][booking.venue_type] = parseInt(slots_available[booking.slot_time][booking.venue_type] - 1*((booking.courts == null || booking.courts == undefined) ? 1 : booking.courts ))
        }
        return slots_available
      })
      available_inventory = available_inventory[available_inventory.length-1]
