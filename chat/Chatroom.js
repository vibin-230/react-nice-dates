...
module.exports = function ({ _id, image }) {
  ...
  async function getChatHistory(id, token) {
    ...
    const x = await Conversation.findById({ _id: id }).lean().then((conversation) => {
      ...
      return Message.find(filter).lean().populate('author', 'name _id').populate('user', 'name _id profile_picture phone').populate({ path: 'game', populate: { path: 'conversation' , populate :{path:'last_message'} } }).sort({_id:-1}).limit(20).then(m => {
        return m.reverse()
      }).catch((e) => console.log(e))
    }).catch((e) => console.log(e))
