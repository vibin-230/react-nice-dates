...
module.exports = function ({ _id, image }) {
  ...
  async function getChatHistory(id, token) {
    ...
    const x = await Conversation.findById({ _id: id }).populate("members","name profile_picture handle name_status visibility").populate("host","name profile_picture handle name_status visibility").populate("to","name profile_picture handle name_status visibility").lean().then((conversation) => {
      ...
      return Message.find(filter).lean().populate('author', 'name _id handle').populate('user', 'name _id profile_picture phone handle visibility').populate({path:"event",populate:{path:"venue",select:"venue"}}).populate({ path: 'game', populate: { path: 'conversation' , populate :{path:'last_message'} } }).sort({_id:-1}).limit(20).then(m => {
        m.splice(0,0,{conversation:conversation._id,message:parseDate(moment(m[0].created_at).utc().format('MM-DD-YYYY')),name:'bot',read_status:false,read_by:user.id,author:user.id,type:'bot',created_at:m[0].created_at,time:true})
